-- 人声增强（STFT 频域版）
--
-- 与 vocal_remove_fft.lua 同结构：宿主 engine.rfft / engine.irfft / engine.spec_gain / engine.vmul；
-- Lua 只对 Mid 做人声带增益抬升。建议关闭自带 EQ。
...
local function rebuild(sr)
  ...
  for k = 0, BINS - 1 do
    ...
    if freq < cfg.vocal_lo then
      ...
      g = cfg.mid_gain_bass + (cfg.mid_gain_vocal - cfg.mid_gain_bass) * t
    elseif freq <= cfg.vocal_hi then
      g = cfg.mid_gain_vocal
    else
      local t = math.min(1.0, (freq - cfg.vocal_hi) / math.max(cfg.vocal_hi, 1))
      g = cfg.mid_gain_vocal + (cfg.mid_gain_air - cfg.mid_gain_vocal) * t
    end
    keep[k + 1] = g
  end
